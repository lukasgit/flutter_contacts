...
public class ContactsServicePlugin implements MethodCallHandler, FlutterPlugin, ActivityAware {
    ...
    switch(call.method){
      case "getContacts": {
          this.getContacts(
              ...
              (List<String>) call.argument("contactInfos"),
              result);
        break;
        ...
        if (this.deleteContact(contact)) {
          result.success(null);
            ...
            result.error(
                null, "Failed to delete the contact, make sure it has a valid identifier", null);
        }
        break;
      } case "updateContact": {
        final Contact contact = Contact.fromMap((HashMap)call.arguments);
        if (this.updateContact(contact)) {
          result.success(null);
            ...
            result.error(
                null, "Failed to update the contact, make sure it has a valid identifier", null);
        }
        break;
  ...
  private void getContacts(
      ...
      boolean photoHighResolution,
      boolean orderByGivenName,
      boolean localizedLabels,
      List<String> contactInfos,
      Result result) {
    new GetContactsTask(
            callMethod,
            result,
  ...
  private ArrayList<Contact> getContacts(
      String query, String rawContactId, boolean localizedLabels, List<String> contactInfos) {
    String selection =
        "("
    ...
    return queryContacts(selection, selectionArgs, localizedLabels, contactInfos);
  }

  private void getContactsForPhone(
      String callMethod,
      String phone,
      boolean withThumbnails,
      boolean photoHighResolution,
    ...
    new GetContactsTask(
        ...
        .executeOnExecutor(executor, phone, true);
  }
  ...
  private class GetContactsTask extends AsyncTask<Object, Void, ArrayList<HashMap>> {
    ...
    protected ArrayList<HashMap> doInBackground(Object... params) {
      ...
      switch (callMethod) {
        case "openDeviceContactPicker":
          contacts = getContacts(null, (String) params[0], localizedLabels, contactInfos);
          break;
        case "getContacts":
          contacts = getContacts((String) params[0], null, localizedLabels, contactInfos);
          break;
        case "getContactsForPhone":
...
//              setAvatarDataForContactIfAvailable(c);
  ...
  private ArrayList<Contact> queryContacts(
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.account")) {
      Cursor accountInfoCursor =
      ...
      while (accountInfoCursor != null && accountInfoCursor.moveToNext()) {
        populateAccountInfo(map, accountInfoCursor);
      }
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.name")) {
      Cursor nameCursor =
          contentResolver.query(
              ContactsContract.Data.CONTENT_URI,
              NAME_PROJECTION,
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.note")) {
      Cursor noteCursor =
          contentResolver.query(
              ContactsContract.Data.CONTENT_URI,
              NOTE_PROJECTION,
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.phone")) {
      Cursor phoneCursor =
          contentResolver.query(
              ContactsContract.Data.CONTENT_URI,
              PHONE_PROJECTION,
      ...
      while (phoneCursor != null && phoneCursor.moveToNext()) {
        populatePhone(map, phoneCursor, localizedLabels);
      }
      if (phoneCursor != null) {
        phoneCursor.close();
      }
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.email")) {
      Cursor emailCursor =
          contentResolver.query(
              ContactsContract.Data.CONTENT_URI,
              EMAIL_PROJECTION,
      ...
      while (emailCursor != null && emailCursor.moveToNext()) {
        populateEmail(map, emailCursor, localizedLabels);
      }
      if (emailCursor != null) {
        emailCursor.close();
      }
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.org")) {
      Cursor orgCursor =
          contentResolver.query(
              ContactsContract.Data.CONTENT_URI,
              ORG_PROJECTION,
      ...
      while (orgCursor != null && orgCursor.moveToNext()) {
        populateOrg(map, orgCursor);
      }
      if (orgCursor != null) {
        orgCursor.close();
      }
    ...
    if (contactInfos == null || contactInfos.contains("ContactInfo.location")) {
      ...
      while (locationCursor != null && locationCursor.moveToNext()) {
        populateLocation(map, locationCursor, localizedLabels);
      }
  ...
  private static void populateAccountInfo(HashMap<String, Contact> map, Cursor accountInfoCursor) {
    int columnIndex = accountInfoCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    String contactId = accountInfoCursor.getString(columnIndex);
    ...
    contact.displayName =
        accountInfoCursor.getString(
            accountInfoCursor.getColumnIndex(ContactsContract.Contacts.DISPLAY_NAME));
    contact.androidAccountType =
        accountInfoCursor.getString(
            accountInfoCursor.getColumnIndex(ContactsContract.RawContacts.ACCOUNT_TYPE));
    contact.androidAccountName =
        accountInfoCursor.getString(
            accountInfoCursor.getColumnIndex(ContactsContract.RawContacts.ACCOUNT_NAME));
  }

  private static void populateName(HashMap<String, Contact> map, Cursor nameCursor) {
    int columnIndex = nameCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    String contactId = nameCursor.getString(columnIndex);
    ...
    String mimeType =
        nameCursor.getString(nameCursor.getColumnIndex(ContactsContract.Data.MIMETYPE));
    if (mimeType.equals(CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE)) {
      contact.givenName =
          nameCursor.getString(nameCursor.getColumnIndex(StructuredName.GIVEN_NAME));
      contact.middleName =
          nameCursor.getString(nameCursor.getColumnIndex(StructuredName.MIDDLE_NAME));
      contact.familyName =
          nameCursor.getString(nameCursor.getColumnIndex(StructuredName.FAMILY_NAME));
  ...
  private static void populateNote(HashMap<String, Contact> map, Cursor noteCursor) {
    int columnIndex = noteCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    String contactId = noteCursor.getString(columnIndex);
    ...
    String mimeType =
        noteCursor.getString(noteCursor.getColumnIndex(ContactsContract.Data.MIMETYPE));
    if (mimeType.equals(CommonDataKinds.Note.CONTENT_ITEM_TYPE)) {
      contact.note = noteCursor.getString(noteCursor.getColumnIndex(CommonDataKinds.Note.NOTE));
    }
  }

  private void populatePhone(
      HashMap<String, Contact> map, Cursor phoneCursor, boolean localizedLabels) {
    int columnIndex = phoneCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    ...
    String mimeType =
        phoneCursor.getString(phoneCursor.getColumnIndex(ContactsContract.Data.MIMETYPE));
    if (mimeType.equals(CommonDataKinds.Phone.CONTENT_ITEM_TYPE)) {
      ...
      if (!TextUtils.isEmpty(phoneNumber)) {
        ...
        String label = Item.getPhoneLabel(resources, type, phoneCursor, localizedLabels);
  ...
  private void populateEmail(
      HashMap<String, Contact> map, Cursor emailCursor, boolean localizedLabels) {
    int columnIndex = emailCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    String contactId = emailCursor.getString(columnIndex);
    ...
    String mimeType =
        emailCursor.getString(emailCursor.getColumnIndex(ContactsContract.Data.MIMETYPE));
    if (mimeType.equals(CommonDataKinds.Email.CONTENT_ITEM_TYPE)) {
      ...
      if (!TextUtils.isEmpty(email)) {
        String label = Item.getEmailLabel(resources, type, emailCursor, localizedLabels);
        contact.emails.add(new Item(label, email, type));
      }
    }
  }

  private static void populateOrg(HashMap<String, Contact> map, Cursor orgCursor) {
    int columnIndex = orgCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    String contactId = orgCursor.getString(columnIndex);
    ...
    String mimeType = orgCursor.getString(orgCursor.getColumnIndex(ContactsContract.Data.MIMETYPE));
    if (mimeType.equals(CommonDataKinds.Organization.CONTENT_ITEM_TYPE)) {
      contact.company = orgCursor.getString(orgCursor.getColumnIndex(Organization.COMPANY));
      contact.jobTitle = orgCursor.getString(orgCursor.getColumnIndex(Organization.TITLE));
    }
  }

  private void populateLocation(
      HashMap<String, Contact> map, Cursor locationCursor, boolean localizedLabels) {
    int columnIndex = locationCursor.getColumnIndex(ContactsContract.Data.CONTACT_ID);
    String contactId = locationCursor.getString(columnIndex);
    ...
    String mimeType =
        locationCursor.getString(locationCursor.getColumnIndex(ContactsContract.Data.MIMETYPE));
    if (mimeType.equals(CommonDataKinds.StructuredPostal.CONTENT_ITEM_TYPE)) {
      int type = locationCursor.getInt(locationCursor.getColumnIndex(StructuredPostal.TYPE));
      String label = PostalAddress.getLabel(resources, type, locationCursor, localizedLabels);
      String street =
          locationCursor.getString(locationCursor.getColumnIndex(StructuredPostal.STREET));
      String city = locationCursor.getString(locationCursor.getColumnIndex(StructuredPostal.CITY));
      String postcode =
          locationCursor.getString(locationCursor.getColumnIndex(StructuredPostal.POSTCODE));
      String region =
          locationCursor.getString(locationCursor.getColumnIndex(StructuredPostal.REGION));
      String country =
          locationCursor.getString(locationCursor.getColumnIndex(StructuredPostal.COUNTRY));
  ...
  private ArrayList<Contact> getContactsFrom(Cursor cursor, boolean localizedLabels) {
    ...
    while (cursor != null && cursor.moveToNext()) {
      ...
      populateAccountInfo(map, cursor);
      // NAMES
      populateName(map, cursor);
      // NOTE
      populateNote(map, cursor);
      // PHONES
      populatePhone(map, cursor, localizedLabels);
      // MAILS
      populateEmail(map, cursor, localizedLabels);
      // ORG
      populateOrg(map, cursor);
      // ADDRESSES
      populateLocation(map, cursor, localizedLabels);
      // BIRTHDAY
      if (mimeType.equals(CommonDataKinds.Event.CONTENT_ITEM_TYPE)) {
        ...
        if (eventType == CommonDataKinds.Event.TYPE_BIRTHDAY) {
          contact.birthday =
              cursor.getString(cursor.getColumnIndex(CommonDataKinds.Event.START_DATE));
        }
      }
    }
  ...
  private void getAvatar(final Contact contact, final boolean highRes, final Result result) {
  ...
  private boolean addContact(Contact contact){
    ...
    for(Item phone : contact.phones){
      ...
      } else op.withValue(ContactsContract.CommonDataKinds.Phone.TYPE, phone.type);
